...
public class ProjectModelTest {
    ...
    @Test public void codecRoundTripsHierarchyAndRussianText(){
        ProjectModel p=new ProjectModel("abc","Съёмка",10);ProjectModel.Group g=new ProjectModel.Group("g","Связка 12");ProjectModel.Scan s=new ProjectModel.Scan("s","Скан №1"),other=new ProjectModel.Scan("o","Другой");s.uri="content://точки/1";s.transform[3]=42;g.expanded=false;p.root.add(g);g.add(s);p.root.add(other);p.setReference(g);p.setMoving(other);String encoded=ProjectCodec.encode(p);ProjectModel restored=ProjectCodec.decode(encoded);assertEquals("Съёмка",restored.name);assertEquals(2,restored.scanCount());ProjectModel.Group rg=(ProjectModel.Group)restored.findNode("g");ProjectModel.Scan rs=(ProjectModel.Scan)restored.findNode("s");assertEquals(s.uri,rs.uri);assertEquals(42,rs.transform[3],T);assertFalse(rg.expanded);assertEquals("g",restored.referenceNodeId);assertEquals("o",restored.movingNodeId);
    }
