...
public final class PointCloudView extends GLSurfaceView {
    ...
    private static final class CloudRenderer implements GLSurfaceView.Renderer {
        ...
        CloudRenderer(PerformanceListener performanceListener,ClipReporter clipReporter,float density){this.performanceListener=performanceListener;this.clipReporter=clipReporter;clipHandleRadius=36f*density;}
        ...
        float[] sampledClipBounds(){long total=0;for(SceneCloud scene:sceneClouds.values())if(scene.visible)for(FloatBuffer chunk:scene.chunks)total+=chunk.capacity()/3;if(total<2)return null;long stride=Math.max(1,(total+199_999)/200_000);int capacity=(int)Math.min(200_000,(total+stride-1)/stride);float[] sample=new float[capacity*3];int output=0;long index=0;for(SceneCloud scene:sceneClouds.values())if(scene.visible){double radians=Math.toRadians(scene.pose[3]),cos=Math.cos(radians),sin=Math.sin(radians);for(FloatBuffer chunk:scene.chunks)for(int i=0;i<chunk.capacity();i+=3,index++){if(index%stride!=0||output>=sample.length)continue;float x=chunk.get(i),y=chunk.get(i+1),z=chunk.get(i+2);sample[output++]=(float)(cos*x-sin*y)+scene.pose[0];sample[output++]=(float)(sin*x+cos*y)+scene.pose[1];sample[output++]=z+scene.pose[2];}}if(output<6)return null;if(output!=sample.length)sample=java.util.Arrays.copyOf(sample,output);try{return ClipBoxMath.centralBounds(sample,.15f,.85f);}catch(IllegalArgumentException ignored){float[] fallback=sceneFrame.clone();return ClipBoxMath.valid(fallback)?fallback:null;}}
