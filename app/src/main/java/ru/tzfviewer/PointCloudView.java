...
public final class PointCloudView extends GLSurfaceView {
    ...
    void toggleProjection(){if(topDownManipulator)return;queueEvent(()->renderer.orthographic=!renderer.orthographic);requestRender();}
    ...
    private static final class CloudRenderer implements GLSurfaceView.Renderer {
        ...
        void panByPixels(float dx,float dy){if(width<=0||height<=0)return;float d=orthographic?4f:3.5f/zoom;float unitsPerPixel=(orthographic?4f/zoom:2f*d*(float)Math.tan(Math.toRadians(22.5)))/height;float[] delta=ScreenPanMath.delta(dx,dy,unitsPerPixel,yaw,pitch);panX+=delta[0];panY+=delta[1];panZ+=delta[2];}
