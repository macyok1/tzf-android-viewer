...
public final class PointCloudView extends GLSurfaceView {
    ...
    void setActiveSceneTarget(String[] scanIds,float[] worldTransform){String[] ids=scanIds.clone();float[] pose=worldTransform==null?null:worldTransform.clone();queueEvent(()->renderer.setActiveSceneTarget(ids,pose));requestRender();}
    ...
    private static final class CloudRenderer implements GLSurfaceView.Renderer {
        ...
        void setSceneCloudTransform(String id,float[] pose){SceneCloud c=sceneClouds.get(id);if(c!=null){System.arraycopy(pose,0,c.pose,0,Math.min(4,pose.length));updateSceneFrame();}}
        void setActiveSceneTarget(String[] ids,float[] pose){activeSceneIds.clear();java.util.Collections.addAll(activeSceneIds,ids);if(pose==null){java.util.Arrays.fill(transform,0);gizmo.endDrag();}else System.arraycopy(pose,0,transform,0,Math.min(4,pose.length));updateActivePivot();}
        ...
        void fitVisible(){updateSceneFrame();zoom=1f;}
        void updateActivePivot(){float[] frame=null;for(Map.Entry<String,SceneCloud> entry:sceneClouds.entrySet())if(activeSceneIds.contains(entry.getKey())&&entry.getValue().localBounds!=null)frame=SceneBounds.merge(frame,SceneBounds.transformed(entry.getValue().localBounds,entry.getValue().pose));if(frame==null)return;gizmoPivot[0]=(frame[0]+frame[3])*.5f;gizmoPivot[1]=(frame[1]+frame[4])*.5f;gizmoPivot[2]=(frame[2]+frame[5])*.5f;}
        ...
        @Override public void onDrawFrame(javax.microedition.khronos.opengles.GL10 gl){
            ...
            if(!activeSceneIds.isEmpty())drawGizmo();
        ...
        void beginGizmo(float x,float y){if(activeSceneIds.isEmpty()||!inverseMvpValid){gizmo.endDrag();return;}updateGizmoPivot();gizmo.beginDrag(x,y,transform,gizmoPivot,primaryMvp,inversePrimaryMvp,width,height);}
        float[] moveGesture(float dx,float dy,float x,float y){if(gizmo.activeHandle()!=TransformGizmo.NONE){float[] next=gizmo.updateDrag(x,y,inversePrimaryMvp,width,height);if(next!=null){System.arraycopy(next,0,transform,0,Math.min(4,next.length));return transform.clone();}return null;}yaw=ViewCubeMath.normalizeYaw(yaw+dx*.35f);pitch=ViewCubeMath.clampPitch(pitch+dy*.35f);return null;}
