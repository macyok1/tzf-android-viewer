...
public final class PointCloudView extends GLSurfaceView {
    ...
    private Runnable pendingSurfaceReadyAction;
    private boolean surfaceReady;
    private OrientationListener orientationListener;
    private ValueAnimator cameraAnimator;
    private float uiYaw=25f,uiPitch=-18f;
    private float savedManipulatorYaw,savedManipulatorPitch;
    private boolean topDownManipulator;
    ...
    public PointCloudView(Context context,AttributeSet attrs){
        super(context);touchSlop=ViewConfiguration.get(context).getScaledTouchSlop();ActivityManager manager=(ActivityManager)context.getSystemService(Context.ACTIVITY_SERVICE);boolean gles3=manager!=null&&manager.getDeviceConfigurationInfo().reqGlEsVersion>=0x30000;setEGLContextClientVersion(gles3?3:2);renderer=new CloudRenderer(fps->{PerformanceListener listener=performanceListener;if(listener!=null)post(()->listener.onFrameRate(fps));},(bounds,frame,commit)->post(()->{ClipListener listener=clipListener;if(listener!=null)listener.onClipChanged(bounds,frame,commit);}),context.getResources().getDisplayMetrics().density,(id,message)->post(()->{SceneErrorListener listener=sceneErrorListener;if(listener!=null)listener.onSceneError(id,message);}),()->post(()->{Runnable listener=contextLossListener;if(listener!=null)listener.run();}),()->post(this::markSurfaceReady));longPressAction=()->{longPressTriggered=true;float x=downX,y=downY;queueEvent(()->renderer.recenterClipAt(x,y));requestRender();};setRenderer(renderer);setRenderMode(RENDERMODE_WHEN_DIRTY);
    ...
    void runWhenSurfaceReady(Runnable action){if(surfaceReady)post(action);else pendingSurfaceReadyAction=action;}
    private void markSurfaceReady(){surfaceReady=true;Runnable action=pendingSurfaceReadyAction;pendingSurfaceReadyAction=null;if(action!=null)action.run();}
    ...
    void beginSceneCloudReplacement(String id,float[] transform,int color,boolean visible,float[] localBounds){float[] pose=transform.clone(),bounds=localBounds==null?null:localBounds.clone();queueEvent(()->renderer.beginReplacement(id,pose,color,visible,bounds));requestRender();}
    void appendSceneCloudReplacement(String id,float[] xyz)throws InterruptedException{uploadPermits.acquire();try{queueEvent(()->{try{renderer.appendReplacement(id,xyz);}finally{uploadPermits.release();}});requestRender();}catch(RuntimeException error){uploadPermits.release();throw error;}}
    void commitSceneCloudReplacement(String id){queueEvent(()->renderer.commitReplacement(id));requestRender();}
    void cancelSceneCloudReplacement(String id){queueEvent(()->renderer.cancelReplacement(id));requestRender();}
    void setSceneUpdatesFrozen(boolean frozen){queueEvent(()->renderer.setSceneUpdatesFrozen(frozen));requestRender();}
    ...
    void restoreOrientation(float yaw,float pitch){if(topDownManipulator)return;cancelCameraAnimation();applyOrientation(yaw,pitch);}
    void rotateCamera(float deltaYaw,float deltaPitch){if(topDownManipulator)return;cancelCameraAnimation();applyOrientation(uiYaw+deltaYaw,uiPitch+deltaPitch);}
    ...
    void setOrthographic(boolean orthographic){if(topDownManipulator)return;queueEvent(()->renderer.orthographic=orthographic);requestRender();}
    void setTopDownManipulator(boolean enabled){
    ...
    private static final class CloudRenderer implements GLSurfaceView.Renderer {
        ...
        private final Runnable surfaceReadyReporter;
        private final float clipHandleRadius;
        private final float[] projection=new float[16],view=new float[16],viewProjection=new float[16],inverseViewProjection=new float[16],commonModel=new float[16],primaryMv=new float[16],primaryMvp=new float[16],inversePrimaryMvp=new float[16];
        ...
        private final float[] transform=new float[4],projectIn=new float[4],projectOut=new float[4],cameraTargetClip=new float[4],gizmoPivot=new float[3],screenA=new float[2],screenB=new float[2],faceCenter=new float[3],combinedClipBoundsScratch=new float[6];
        ...
        private boolean manipulatorMode,savedOrthographic;
        private float savedYaw,savedPitch,savedZoom,savedPanX,savedPanY,savedPanZ;
        private long fpsStarted; private int fpsFrames;
        private boolean surfaceCreated;
        private boolean sceneUpdatesFrozen;
        boolean orthographic=true,gridVisible=true;
        float yaw=25f,pitch=-18f,zoom=1f,pointSize=2f;

        CloudRenderer(PerformanceListener performanceListener,ClipReporter clipReporter,float density,SceneFailureReporter failureReporter,Runnable contextLossReporter,Runnable surfaceReadyReporter){this.performanceListener=performanceListener;this.clipReporter=clipReporter;this.failureReporter=failureReporter;this.contextLossReporter=contextLossReporter;this.surfaceReadyReporter=surfaceReadyReporter;clipHandleRadius=36f*density;}
        ...
        void beginReplacement(String id,float[] pose,int argb,boolean visible,float[] metadataBounds){failedSceneIds.remove(id);SceneCloud c=sceneClouds.get(id);if(c==null){c=new SceneCloud();sceneClouds.put(id,c);}c.clearPending();System.arraycopy(pose,0,c.pose,0,Math.min(4,pose.length));c.r=((argb>>16)&255)/255f;c.g=((argb>>8)&255)/255f;c.b=(argb&255)/255f;c.visible=visible;c.pendingBounds=metadataBounds;c.pendingBoundsFixed=metadataBounds!=null;c.pendingCommit=false;}
        void appendReplacement(String id,float[] xyz){SceneCloud c=sceneClouds.get(id);if(c==null||failedSceneIds.contains(id))return;try{GpuPointChunk chunk=uploadChunk(xyz);c.pendingChunks.add(chunk);if(!c.pendingBoundsFixed)c.pendingBounds=SceneBounds.merge(c.pendingBounds,SceneBounds.of(xyz));c.pendingSample.add(xyz);}catch(RuntimeException error){c.clearPending();failedSceneIds.add(id);failureReporter.report(id,error.getMessage()==null?"ошибка GPU":error.getMessage());}}
        GpuPointChunk uploadChunk(float[] xyz){while(GLES20.glGetError()!=GLES20.GL_NO_ERROR){}FloatBuffer upload=direct(xyz.length);upload.put(xyz).position(0);int[] generated=new int[1];GLES20.glGenBuffers(1,generated,0);if(generated[0]==0)throw new IllegalStateException("GPU не создал буфер точек");GLES20.glBindBuffer(GLES20.GL_ARRAY_BUFFER,generated[0]);GLES20.glBufferData(GLES20.GL_ARRAY_BUFFER,xyz.length*4,upload,GLES20.GL_STATIC_DRAW);int error=GLES20.glGetError();GLES20.glBindBuffer(GLES20.GL_ARRAY_BUFFER,0);if(error!=GLES20.GL_NO_ERROR){GLES20.glDeleteBuffers(1,generated,0);throw new IllegalStateException(error==GLES20.GL_OUT_OF_MEMORY?"недостаточно памяти GPU — выберите 10M или AUTO":"ошибка загрузки GPU: "+error);}return new GpuPointChunk(generated[0],xyz.length/3);}
        void commitReplacement(String id){SceneCloud c=sceneClouds.get(id);if(c==null)return;if(sceneUpdatesFrozen){c.pendingCommit=true;return;}activatePending(c);updateSceneFrame();}
        void cancelReplacement(String id){SceneCloud c=sceneClouds.get(id);if(c!=null)c.clearPending();}
        void setSceneUpdatesFrozen(boolean frozen){if(sceneUpdatesFrozen==frozen)return;sceneUpdatesFrozen=frozen;if(!frozen){boolean changed=false;for(SceneCloud c:sceneClouds.values())if(c.pendingCommit){activatePending(c);changed=true;}if(changed)updateSceneFrame();}}
        void activatePending(SceneCloud c){if(c.pendingChunks.isEmpty())return;c.delete(c.chunks);c.chunks.clear();c.chunks.addAll(c.pendingChunks);c.pendingChunks.clear();c.sample=c.pendingSample;c.pendingSample=new PointReservoir(200_000);c.localBounds=c.pendingBounds;c.pendingBounds=null;c.pendingCommit=false;}
        ...
        void setSceneCloudTransform(String id,float[] pose){SceneCloud c=sceneClouds.get(id);if(c!=null){System.arraycopy(pose,0,c.pose,0,Math.min(4,pose.length));if(frameLockedForGizmo||manipulatorMode)updateActivePivot();else updateSceneFrame();}}
        ...
        void enterManipulator(){if(manipulatorMode)return;savedYaw=yaw;savedPitch=pitch;savedZoom=zoom;savedPanX=panX;savedPanY=panY;savedPanZ=panZ;savedOrthographic=orthographic;manipulatorMode=true;gizmo.setPlanarOnly(true);orthographic=true;yaw=0;pitch=-90;updateActivePivot();panX=(gizmoPivot[0]-cx)*2f/span;panY=(gizmoPivot[1]-cy)*2f/span;panZ=(gizmoPivot[2]-cz)*2f/span;}
        void exitManipulator(){if(!manipulatorMode)return;manipulatorMode=false;gizmo.setPlanarOnly(false);updateSceneFrame();yaw=savedYaw;pitch=savedPitch;zoom=savedZoom;panX=savedPanX;panY=savedPanY;panZ=savedPanZ;orthographic=savedOrthographic;}
        ...
        @Override public void onSurfaceCreated(javax.microedition.khronos.opengles.GL10 gl,javax.microedition.khronos.egl.EGLConfig cfg){boolean contextRestored=surfaceCreated;surfaceCreated=true;if(contextRestored){sceneClouds.clear();failedSceneIds.clear();contextLossReporter.run();}GLES20.glClearColor(.015f,.025f,.04f,1);GLES20.glEnable(GLES20.GL_DEPTH_TEST);program=link(VS,FS);position=GLES20.glGetAttribLocation(program,"aPosition");matrix=GLES20.glGetUniformLocation(program,"uMvp");world=GLES20.glGetUniformLocation(program,"uWorld");color=GLES20.glGetUniformLocation(program,"uColor");size=GLES20.glGetUniformLocation(program,"uSize");clipMin=GLES20.glGetUniformLocation(program,"uClipMin");clipMax=GLES20.glGetUniformLocation(program,"uClipMax");clip=GLES20.glGetUniformLocation(program,"uClip");Matrix.setIdentityM(identity,0);surfaceReadyReporter.run();}
        ...
        void drawGizmo(){float s=gizmo.worldScale(),x=gizmoPivot[0],y=gizmoPivot[1],z=gizmoPivot[2];FloatBuffer b=gizmoBuffer;GLES20.glLineWidth(5);if(!manipulatorMode){drawGizmoLine(b,x,y,z,x+s,y,z,TransformGizmo.X,1,.15f,.15f);drawGizmoLine(b,x,y,z,x,y+s,z,TransformGizmo.Y,.15f,1,.3f);drawGizmoLine(b,x,y,z,x,y,z+s,TransformGizmo.Z,.2f,.5f,1);}b.position(0);float c=s*(manipulatorMode?.16f:.12f);put(b,x-c,y,z,x+c,y,z,x,y-c,z,x,y+c,z);b.position(0);float xy=gizmo.activeHandle()==TransformGizmo.XY?1f:.72f;draw(b,4,GLES20.GL_LINES,primaryMvp,xy,xy,xy,1);b.position(0);int seg=48;float ring=s*(manipulatorMode?1f:.75f);for(int i=0;i<seg;i++){double a=i*Math.PI*2/seg,n=(i+1)*Math.PI*2/seg;put(b,x+(float)Math.cos(a)*ring,y+(float)Math.sin(a)*ring,z,x+(float)Math.cos(n)*ring,y+(float)Math.sin(n)*ring,z);}b.position(0);float hi=gizmo.activeHandle()==TransformGizmo.RZ?1f:.75f;draw(b,seg*2,GLES20.GL_LINES,primaryMvp,1,hi,.15f,1);}
        ...
        void recenterClipAt(float x,float y){if(manipulatorMode||!clipping||!clipControlsVisible||activeClipFace>=0||frameLockedForGizmo)return;float[] point=nearestWorldPoint(x,y);if(point==null)return;float[] moved=ClipBoxMath.recentered(combinedClipBounds(),point[0],point[1],point[2]);System.arraycopy(moved,0,clipLower,0,3);System.arraycopy(moved,3,clipUpper,0,3);reportClip(true);}
        void resetPerformanceSample(){fpsStarted=0;fpsFrames=0;}
        void panByPixels(float dx,float dy){if(width<=0||height<=0)return;setMatrices();float ndcZ=cameraTargetClip[3]==0?0:cameraTargetClip[2]/cameraTargetClip[3];float[] delta=ScreenPanMath.fromInverseViewProjection(dx,dy,width,height,ndcZ,inverseViewProjection);panX+=delta[0];panY+=delta[1];panZ+=delta[2];}
        ...
        private static final class SceneCloud{final java.util.List<GpuPointChunk> chunks=new java.util.ArrayList<>(),pendingChunks=new java.util.ArrayList<>();PointReservoir sample=new PointReservoir(200_000),pendingSample=new PointReservoir(200_000);float[] localBounds,pendingBounds;boolean pendingCommit,pendingBoundsFixed,visible=true;float r=.2f,g=.8f,b=1f;final float[] pose=new float[4];void delete(java.util.List<GpuPointChunk> list){if(!list.isEmpty()){int[] buffers=new int[list.size()];for(int i=0;i<list.size();i++)buffers[i]=list.get(i).buffer;GLES20.glDeleteBuffers(buffers.length,buffers,0);}}void clearPending(){delete(pendingChunks);pendingChunks.clear();pendingSample.clear();pendingBounds=null;pendingCommit=false;pendingBoundsFixed=false;}void clearGpu(){delete(chunks);chunks.clear();sample.clear();localBounds=null;clearPending();}}
    }
