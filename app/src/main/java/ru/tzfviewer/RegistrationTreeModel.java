...
final class RegistrationTreeModel {
    ...
    enum Action { ACCEPT, REJECT, RETRY, MANUAL_REGISTER, SET_REFERENCE, SET_MOVING }
    ...
    static List<Row> rows(ProjectModel project){List<Row> rows=new ArrayList<>();boolean multipleSets=project.registrationSets.size()>1;for(ProjectModel.RegistrationSet set:project.registrationSets){rows.add(new Row(Kind.SET,set.id,"",set.name,set.stationIds.size()+" станц.",Tone.GRAY,EnumSet.noneOf(Action.class)));for(String stationId:set.stationIds){ProjectModel.Node node=project.findNode(stationId);if(!(node instanceof ProjectModel.Scan))continue;ProjectModel.Scan scan=(ProjectModel.Scan)node;EnumSet<Action> actions=EnumSet.noneOf(Action.class);if(scan.registrationState==ProjectModel.RegistrationState.CHECK){actions.add(Action.ACCEPT);actions.add(Action.REJECT);}else if(scan.registrationState==ProjectModel.RegistrationState.FAILED){if(!scan.attemptedReferenceId.isEmpty())actions.add(Action.RETRY);actions.add(Action.MANUAL_REGISTER);}else if(multipleSets&&(scan.registrationState==ProjectModel.RegistrationState.UNLINKED||scan.registrationState==ProjectModel.RegistrationState.LEGACY_UNLINKED))actions.add(Action.MANUAL_REGISTER);rows.add(new Row(Kind.STATION,set.id,scan.id,scan.name,label(scan.registrationState),tone(scan.registrationState),actions));}}return rows;}
