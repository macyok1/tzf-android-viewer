...
final class ProjectCodec {
    ...
    private static void writeNode(StringBuilder out,ProjectModel.Node node,String parent){
        ...
        if(node instanceof ProjectModel.Scan){ProjectModel.Scan s=(ProjectModel.Scan)node;out.append('\t').append(e(s.uri)).append('\t').append(s.color).append('\t').append(s.sourcePointCount).append('\t').append(s.embeddedPoseValid).append('\t').append(s.embeddedPoseApplied);for(float value:s.embeddedPose)out.append('\t').append(value);}
        out.append('\n');
    ...
    static ProjectModel decode(String text){
        String[] lines=text.split("\\r?\\n");if(lines.length<2)throw new IllegalArgumentException("unsupported project format");boolean v4="TZF_PROJECT\t4".equals(lines[0]),v3="TZF_PROJECT\t3".equals(lines[0]),v2="TZF_PROJECT\t2".equals(lines[0]);if(!v4&&!v3&&!v2&&!"TZF_PROJECT\t1".equals(lines[0]))throw new IllegalArgumentException("unsupported project format");
        String[] p=lines[1].split("\\t",-1);if((v4||v3?p.length!=22:v2?p.length!=14:p.length!=12)||!"P".equals(p[0]))throw new IllegalArgumentException("invalid project header");
        ...
        if(v2||v3||v4){project.referenceNodeId=d(p[12]);project.movingNodeId=d(p[13]);}if(v3||v4){project.clipEnabled=Boolean.parseBoolean(p[14]);project.clipLocked=Boolean.parseBoolean(p[15]);for(int i=0;i<6;i++)project.clipBounds[i]=flt(p[16+i]);if(!ClipBoxMath.valid(project.clipBounds)){project.clipEnabled=false;project.clipLocked=false;java.util.Arrays.fill(project.clipBounds,0);}}
        Map<String,ProjectModel.Node> nodes=new LinkedHashMap<>();Map<String,String> parents=new LinkedHashMap<>();
        for(int i=2;i<lines.length;i++){if(lines[i].isEmpty())continue;String[] a=lines[i].split("\\t",-1);int shift=(v2||v3||v4)?1:0;if(a.length<9+shift)throw new IllegalArgumentException("invalid node");ProjectModel.Node node;if("G".equals(a[0]))node=new ProjectModel.Group(d(a[1]),d(a[3]));else if("S".equals(a[0])&&a.length==(v4?19:12+shift)){ProjectModel.Scan s=new ProjectModel.Scan(d(a[1]),d(a[3]));s.uri=d(a[9+shift]);s.color=num(a[10+shift]);s.sourcePointCount=lng(a[11+shift]);if(v4){s.embeddedPoseValid=Boolean.parseBoolean(a[13]);s.embeddedPoseApplied=Boolean.parseBoolean(a[14]);for(int j=0;j<4;j++)s.embeddedPose[j]=flt(a[15+j]);if(!s.embeddedPoseValid)java.util.Arrays.fill(s.embeddedPose,0);}else s.embeddedPoseApplied=true;node=s;}else throw new IllegalArgumentException("invalid node type");node.visible=Boolean.parseBoolean(a[4]);node.expanded=!(v2||v3||v4)||Boolean.parseBoolean(a[5]);for(int j=0;j<4;j++)node.transform[j]=flt(a[5+shift+j]);if(nodes.put(node.id,node)!=null)throw new IllegalArgumentException("duplicate id");parents.put(node.id,d(a[2]));}
