...
final class ProjectCodec {
    ...
    static ProjectModel decode(String text){
        String[] lines=text.split("\\r?\\n");if(lines.length<2)throw new IllegalArgumentException("unsupported project format");boolean v2="TZF_PROJECT\t2".equals(lines[0]);if(!v2&&!"TZF_PROJECT\t1".equals(lines[0]))throw new IllegalArgumentException("unsupported project format");
        String[] p=lines[1].split("\\t",-1);if((v2?p.length!=14:p.length!=12)||!"P".equals(p[0]))throw new IllegalArgumentException("invalid project header");
        ...
        if(v2){project.referenceNodeId=d(p[12]);project.movingNodeId=d(p[13]);}
        Map<String,ProjectModel.Node> nodes=new LinkedHashMap<>();Map<String,String> parents=new LinkedHashMap<>();
        for(int i=2;i<lines.length;i++){if(lines[i].isEmpty())continue;String[] a=lines[i].split("\\t",-1);int shift=v2?1:0;if(a.length<9+shift)throw new IllegalArgumentException("invalid node");ProjectModel.Node node;if("G".equals(a[0]))node=new ProjectModel.Group(d(a[1]),d(a[3]));else if("S".equals(a[0])&&a.length==12+shift){ProjectModel.Scan s=new ProjectModel.Scan(d(a[1]),d(a[3]));s.uri=d(a[9+shift]);s.color=num(a[10+shift]);s.sourcePointCount=lng(a[11+shift]);node=s;}else throw new IllegalArgumentException("invalid node type");node.visible=Boolean.parseBoolean(a[4]);node.expanded=!v2||Boolean.parseBoolean(a[5]);for(int j=0;j<4;j++)node.transform[j]=flt(a[5+shift+j]);if(nodes.put(node.id,node)!=null)throw new IllegalArgumentException("duplicate id");parents.put(node.id,d(a[2]));}
