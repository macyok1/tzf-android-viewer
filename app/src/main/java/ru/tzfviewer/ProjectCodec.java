...
final class ProjectCodec {
    ...
    static ProjectModel decode(String text){
        String[] lines=text.split("\\r?\\n");if(lines.length<2)throw new IllegalArgumentException("unsupported project format");String[] signature=lines[0].split("\\t",-1);if(signature.length!=2||!"TZF_PROJECT".equals(signature[0]))throw new IllegalArgumentException("unsupported project format");int version=num(signature[1]);if(version<1||version>5)throw new IllegalArgumentException("unsupported project format");
        boolean v5=version==5,v4=version==4,v3=version==3,v2=version==2;String[] p=lines[1].split("\\t",-1);int expected=(v5||v4||v3)?22:v2?14:12;if(p.length!=expected||!"P".equals(p[0]))throw new IllegalArgumentException("invalid project header");
        ProjectModel project=new ProjectModel(d(p[1]),d(p[2]),lng(p[3]));project.modifiedAt=lng(p[4]);project.pointBudget=num(p[5]);project.pointSize=num(p[6]);project.cameraYaw=flt(p[7]);project.cameraPitch=flt(p[8]);project.cameraZoom=flt(p[9]);project.orthographic=bool(p[10]);project.gridVisible=bool(p[11]);
        if(v2||v3||v4){project.referenceNodeId=d(p[12]);project.movingNodeId=d(p[13]);}if(v3||v4||v5){project.clipEnabled=bool(p[14]);project.clipLocked=bool(p[15]);for(int i=0;i<6;i++)project.clipBounds[i]=flt(p[16+i]);if(!ClipBoxMath.valid(project.clipBounds)){project.clipEnabled=false;project.clipLocked=false;java.util.Arrays.fill(project.clipBounds,0);}}
        ...
        for(int i=2;i<lines.length;i++){
            ...
            if("RS".equals(a[0])){if(v5)setRecords.add(a);continue;}if("RL".equals(a[0])){if(v5)linkRecords.add(a);continue;}
            ...
            else if("S".equals(a[0])&&a.length==(v5?33:v4?19:12+shift)){ProjectModel.Scan s=new ProjectModel.Scan(d(a[1]),d(a[3]));s.uri=d(a[9+shift]);s.color=num(a[10+shift]);s.sourcePointCount=lng(a[11+shift]);if(v4||v5){s.embeddedPoseValid=bool(a[13]);s.embeddedPoseApplied=bool(a[14]);for(int j=0;j<4;j++)s.embeddedPose[j]=flt(a[15+j]);if(!s.embeddedPoseValid)java.util.Arrays.fill(s.embeddedPose,0);}else s.embeddedPoseApplied=true;if(v5){s.acquiredAt=lng(a[19]);s.registrationState=registrationState(a[20]);if(s.registrationState==ProjectModel.RegistrationState.REGISTERING)s.registrationState=ProjectModel.RegistrationState.PENDING;s.attemptedReferenceId=d(a[21]);s.registrationMessage=d(a[22]);s.registrationMetrics=metrics(a,23);s.pendingCandidateValid=bool(a[28]);for(int j=0;j<4;j++)s.pendingCandidateWorld[j]=flt(a[29+j]);if(!s.pendingCandidateValid)java.util.Arrays.fill(s.pendingCandidateWorld,0);}node=s;}
        ...
        if(v5){for(String[] a:setRecords)readSet(project,a);for(String[] a:linkRecords)readLink(project,a);project.referenceNodeId="";project.movingNodeId="";graph.validate();}
