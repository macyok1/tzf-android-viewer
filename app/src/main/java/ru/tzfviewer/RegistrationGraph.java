...
final class RegistrationGraph {
    ...
    void detachStation(String stationId,long now){
        ProjectModel.Node node=project.findNode(stationId);if(!(node instanceof ProjectModel.Scan))throw new IllegalArgumentException("station missing");ProjectModel.Scan detached=(ProjectModel.Scan)node;ProjectModel.RegistrationSet original=setForScan(stationId);if(original==null||original.stationIds.size()<2)return;int originalIndex=project.registrationSets.indexOf(original);List<String> remaining=new ArrayList<>(original.stationIds);remaining.remove(stationId);Set<String> remainingSet=new HashSet<>(remaining);project.registrationLinks.removeIf(link->stationId.equals(link.movingStationId)||stationId.equals(link.referenceStationId));
        Map<String,List<String>> adjacency=new HashMap<>();for(String id:remaining)adjacency.put(id,new ArrayList<>());for(ProjectModel.RegistrationLink link:project.registrationLinks)if(remainingSet.contains(link.movingStationId)&&remainingSet.contains(link.referenceStationId)){adjacency.get(link.movingStationId).add(link.referenceStationId);adjacency.get(link.referenceStationId).add(link.movingStationId);}
        List<List<String>> components=new ArrayList<>();Set<String> visited=new HashSet<>();for(String start:remaining)if(visited.add(start)){List<String> component=new ArrayList<>(),queue=new ArrayList<>();queue.add(start);for(int i=0;i<queue.size();i++){String current=queue.get(i);component.add(current);for(String next:adjacency.get(current))if(visited.add(next))queue.add(next);}component.sort(Comparator.comparingLong(id->((ProjectModel.Scan)project.findNode(id)).acquiredAt));components.add(component);}
        project.registrationSets.remove(original);List<ProjectModel.RegistrationSet> replacement=new ArrayList<>();for(int i=0;i<components.size();i++){ProjectModel.RegistrationSet set=i==0?new ProjectModel.RegistrationSet(original.id,original.name,original.createdAt):new ProjectModel.RegistrationSet(UUID.randomUUID().toString(),"Набор сшивки "+(project.registrationSets.size()+replacement.size()+1),now);set.modifiedAt=now;set.stationIds.addAll(components.get(i));replacement.add(set);for(String id:set.stationIds){ProjectModel.Scan scan=(ProjectModel.Scan)project.findNode(id);scan.registrationState=set.stationIds.size()>1?ProjectModel.RegistrationState.REGISTERED:ProjectModel.RegistrationState.UNLINKED;}}
        ProjectModel.RegistrationSet detachedSet=new ProjectModel.RegistrationSet(UUID.randomUUID().toString(),"Набор сшивки "+(project.registrationSets.size()+replacement.size()+1),now);detachedSet.stationIds.add(stationId);replacement.add(detachedSet);project.registrationSets.addAll(Math.max(0,Math.min(originalIndex,project.registrationSets.size())),replacement);detached.registrationState=ProjectModel.RegistrationState.UNLINKED;detached.attemptedReferenceId="";detached.registrationMessage="";detached.registrationMetrics=ProjectModel.RegistrationMetrics.empty();detached.pendingCandidateValid=false;java.util.Arrays.fill(detached.pendingCandidateWorld,0);for(ProjectModel.Scan scan:allScans())if(stationId.equals(scan.attemptedReferenceId)){scan.attemptedReferenceId="";scan.pendingCandidateValid=false;java.util.Arrays.fill(scan.pendingCandidateWorld,0);}validate();
    }
