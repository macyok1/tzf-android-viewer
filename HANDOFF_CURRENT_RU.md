...
- Глобальный updater перенесён из параметров сшивки в отдельный `SettingsActivity`, открываемый кнопкой `⚙` на экране проектов.
- Состояния updater-кнопки выделены в тестируемую модель; после ошибки кнопка всегда возвращается в активное состояние.
- В workspace добавлена кнопка `←`; она и системный Back сохраняют проект и возвращают к существующему списку проектов. При открытом overlay Back сначала закрывает его.
